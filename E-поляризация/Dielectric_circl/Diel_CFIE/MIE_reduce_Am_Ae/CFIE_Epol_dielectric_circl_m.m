%%  МИУ
% Метод интегральны уравнений для диэл круглого цилиндра
...
for m  = 1 : N
    ...
    % МАГНИТНОЕ ПОЛЕ
        ...
        % cоставляющие E2y
        E2y = 1i*eta2*k2*fe_y2 + eta2*fm_y2;
       
        % cоставляющие H1u
        H1u = 1i/(eta1*k1)*(hm_u11 + (k1^2)*hm_u12)*eta1 - he_u1;
        
        % cоставляющие H2u
        H2u = -1i/(eta2*k2)*(hm_u21 + (k2^2)*hm_u22)*eta2 - he_u2;
        
        % cоставляющие H1w
        H1w = 1i/(eta1*k1)*hm_w1*eta1 + he_w1;
        
        % cоставляющие H2w
        H2w = -1i/(eta2*k2)*hm_w2*eta2 + he_w2;
  
        %% Работа с системоу уравнений 
...
%             Sum_E = Sum_E + eta2*k2/4*dx*j2(n)*exp(cx*xn+cz*zn);
            Sum_E = Sum_E - eta2*k2/4*dx*j2(n)*exp(cx*xn+cz*zn)...
                    - eta2*k2*dx/4*j2(n)*(txn*sind(phi) - tzn*cosd(phi))*exp(cx*xn+cz*zn);
